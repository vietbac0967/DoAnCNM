...
const HomeListFrient = (props) => {
  const { setshowchat, setuser, mdup, user, setpage } = props;
    ...
    handlerefreshAllInfo(async (data) => {
      if (data && data.type === "group") {
        if (dataredux && dataredux.groups.length > 0) {
          let index = dataredux.groups.findIndex(
            (item) => item && item._id === data.groupId
          );
          if (index !== -1) {
        ...
        if (
          dataredux &&
          (dataredux._id === data.sender.userId ||
            dataredux._id === data.receiver.userId)
        ) {
          await handleGeAllFriendUpdate();
        }
      }
  ...
  const handleClick = async (item) => {
    ...
    if (objIndex !== -1) {
      ...
      let objIndexgr = cplistfrient.findIndex(
        (items) => items._id._id === item._id._id
      );
      if (objIndexgr !== -1) {
