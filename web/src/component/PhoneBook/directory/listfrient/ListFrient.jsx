...
const ListFrient = (props) => {
  const { setshowchat, dataredux, setuser, users, page, setpage } = props;
  ...
  useEffect(() => {
    if (dataredux && dataredux.friends && dataredux.friends.length > 0) {
      if (value === +0) {
        setlistuser(
          [...dataredux.friends].sort((a, b) => a.name.localeCompare(b.name))
        );
      } else {
        setlistuser(
          [...dataredux.friends].sort((a, b) => b.name.localeCompare(a.name))
        );
      }
    }
  }, []);
    ...
    <Box className="list-friend-container">
      ...
      <Box className="list-frient-body">
        <Scrollbars style={{ width: "100%", height: "100%" }}>
          ...
          <Box className="list-show-friend">
